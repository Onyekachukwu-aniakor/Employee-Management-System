...
export const createLeave = async (req, res) => {
    try {
        ...
        const leave = await LeaveApplication.create({
         ...
         /* call inngest here from index.js */
    ...
    try {
        ...
        if(isAdmin){
            ...
            const employee = await Employee.findOne({
                userId : session.userId}).lean();
                if(!employee)return res.status(404).json({error : 'Not found'})
                    // if employee is at level
...
export const updateLeaveStatus = async (req, res) => {
    try {
        ...
        //if status is valid
        // 'returnDocument': Has two possible values, 'before' and 'after'. By default, it will return the document before the update was applied.
